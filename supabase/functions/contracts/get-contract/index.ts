...
serve(async (req) => {
  ...
  try {
    ...
    if (contractError || !contract) {
      throw new Error(`Contrat introuvable: ${contractError?.message || 'Contrat non trouvé'}`)
    }
    ...
    if (profile.role === 'client') {
      ...
      const event = contract.quotes?.events
    ...
    return new Response(
      JSON.stringify({ success: false, error: error.message || 'Erreur lors de la récupération du contrat' }),
